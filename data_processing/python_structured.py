...
def main():
    ...
    print(python_context_parse(
        'How to calculateAnd the value of the sum of squares defined as n 1^2 + 2^2 + 3^2 + ... + n^2 until a user-specified sum has been reached sql()'))
    ...
    print(python_code_parse('''
        ...
        def sina_download(url, output_dir='.', merge=True, info_only=False, **kwargs):
            ...
            if vid is None:
                ...
                vid = hd_vid = match1(video_page, r'hd_vids*:s*'([^']+)'')
                if hd_vid == '0':
                    vids = match1(video_page, r'[^w]vids*:s*'([^']+)'').split('|')
                    vid = vids[-1]
            ...
            if vid:
                ...
                vkey = match1(video_page, r'vkeys*:s*"([^"]+)"')
                if vkey is None:
                    vid = match1(url, r'#(d+)')
                    sina_download_by_vid(vid, output_dir=output_dir, merge=merge, info_only=info_only)
                    return
                title = match1(video_page, r'titles*:s*"([^"]+)"')
