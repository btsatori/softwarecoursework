...
def main():
    print(sqlang_code_parse('''
        "geometry": {"type": "Polygon", 111.676, "coordinates": [
            [
                [6.69245274714546, 51.1326962505233], 
                [6.69242714158622, 51.1326908883821], 
                [6.69242919794447, 51.1326955158344], 
                [6.69244041615532, 51.1326998744549], 
                [6.69244125953742, 51.1327001609189], 
                [6.69245274714546, 51.1326962505233]
            ]
        ]}
        How to 123 create a (SQL Server function) to "join" multiple rows from a subquery into a single delimited field?
    '''))
    ...
    print(sqlang_code_parse('''
        ...
        WITH hierarchy AS (
            ...
            SELECT  t.ID, 
                    ...
                    h.ID        
            FROM hierarchy AS h
            JOIN @Table AS t ON t.RequiredID = h.ID
        )
        ...
        WHEN NOT MATCHED THEN
            ...
            VALUES (Source.Code, Source.RequiredID)
        OUTPUT Source.ID, INSERTED.ID INTO @MapIds;

        UPDATE @NewSeed 
        SET RequiredID = aNewID
        FROM @MapIds
